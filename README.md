...
A Flutter app that nudges you to sketch on paper every day. Each day it fetches a fresh reference image from Unsplash, starts a 5-minute countdown, tracks your streak, and celebrates milestone badges.

## Features

- **Daily reminder** — local notification at a configurable time (default 8:00 PM) that survives reboots.
- **Reference image from Unsplash** — rotating query (`portrait`, `still life`, `landscape`, `animal`, `hands`, `architecture`, `flower`) picked by day of year so there's variety without jarring RNG. Image can be refreshed before starting but locks once the timer runs.
- **5-minute timer** with circular countdown ring; pause/resume or finish early. Screen stays on via wakelock during the session.
- **30-day history heatmap** — tap any completed day to replay its original reference image.
- **Streak tracking** (current + longest) with per-day history stored in sqflite.
- **Milestone badges** at 3 / 7 / 14 / 30 / 60 / 100 / 180 / 365 days, with a tiered celebration dialog (subtle → epic confetti) + haptic when unlocked.
...
flutter test      # unit tests for StreakBloc and SketchSessionBloc
```
