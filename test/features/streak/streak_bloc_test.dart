...
import 'package:sketchdaily/features/streak/bloc/streak_bloc.dart';
...
void main() {
  ...
  StreakSessionCompleted completion(DateTime completedAt) =>
  ...
  group('StreakSessionCompleted', () {
    blocTest<StreakBloc, StreakState>(
      'first session starts a streak of 1',
      build: () => StreakBloc(repository: repository),
      act: (bloc) => bloc.add(completion(at(2026, 1, 1))),
      expect: () => [
        isA<StreakState>()
            .having((s) => s.currentStreak, 'current', 1)
            .having((s) => s.longestStreak, 'longest', 1)
            .having((s) => s.completedDates.contains('2026-01-01'), 'has today', true),
      ],
    );
    ...
    blocTest<StreakBloc, StreakState>(
      ...
      seed: () => StreakState(
        currentStreak: 5,
        longestStreak: 5,
        completedDates: {'2025-12-26', '2025-12-27', '2025-12-28', '2025-12-29', '2025-12-30'},
        lastMilestoneCelebrated: 3,
      ),
      act: (bloc) => bloc.add(completion(at(2026, 1, 1))), // gap after 2025-12-30
      expect: () => [
        isA<StreakState>()
            .having((s) => s.currentStreak, 'current', 1)
  ...
  group('StreakResetRequested', () {
    blocTest<StreakBloc, StreakState>(
      ...
      act: (bloc) => bloc.add(const StreakResetRequested()),
